...
Future<void> main() async {
  ...
  runApp(NetWhoApp(settings: settings, scanner: scanner));
  // Show what we remember right away, then refresh it with a live scan.
  await scanner.loadCached();
  scanner.scan();
}

class NetWhoApp extends StatelessWidget {
  const NetWhoApp({super.key, required this.settings, required this.scanner});
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) => MaterialApp(
        ...
        home: HomeShell(settings: settings, scanner: scanner),
      ),
    );
  }
}
...
class HomeShell extends StatefulWidget {
  const HomeShell({super.key, required this.settings, required this.scanner});
...
class _HomeShellState extends State<HomeShell> {
  ...
  Widget build(BuildContext context) {
    final pages = [
      ...
      ToolsPage(scanner: widget.scanner, settings: widget.settings),
    ];
